...
export function detectX01Award(
  darts: { segment: Segment }[],
): X01AwardType | null {
  if (darts.length !== 3) return null;

  if (darts.every((d) => d.segment.Section === SegmentSection.BULL))
    return "hattrick";
...
export function detectCricketAward(
  darts: CricketThrownDart[],
): CricketAwardType | null {
  if (darts.length !== 3) return null;

  // Three in black: all 3 darts are double bullseyes (takes precedence over hattrick)
  if (darts.every((d) => d.segment.ID === SegmentID.DBL_BULL))
    return "threeinblack";
