...
export function createGameStore<TState extends object, TOptions>(
  ...
  type FullState = TState & { undoStack: TState[] } & GameStoreActions<TState, TOptions>;

  const stateKeys = Object.keys(defaultState);

  function extractSnapshot(s: FullState): TState {
    const snapshot: Record<string, unknown> = {};
    for (const key of stateKeys) snapshot[key] = (s as Record<string, unknown>)[key];
  ...
  return create<FullState>()((set, get) => ({
    ...
    startGame: (options: TOptions, playerNames: string[]) =>
      set({ ...engine.startGame(options, playerNames), undoStack: [] } as Partial<FullState>),
    ...
    undoLastDart: () =>
      set((s) => {
        ...
        return { ...prev, undoStack: s.undoStack.slice(0, -1) } as Partial<FullState>;
      }),
