...
function bestOpponentScore(players: CricketPlayer[], myIndex: number): number {
  return Math.max(...players.filter((_, i) => i !== myIndex).map((p) => p.score));
}

/** True if I have this target closed (≥3 marks) */
function iClosed(myMarks: Record<CricketTarget, number>, t: CricketTarget): boolean {
  return myMarks[t] >= 3;
}

/** True if every opponent has this target closed */
function allOpponentsClosed(players: CricketPlayer[], myIndex: number, t: CricketTarget): boolean {
  return players.every((p, i) => i === myIndex || p.marks[t] >= 3);
}

/** True if at least one opponent is still open on this target */
function opponentOpen(players: CricketPlayer[], myIndex: number, t: CricketTarget): boolean {
 ...
 */
function scoringValue(myMarks: Record<CricketTarget, number>, players: CricketPlayer[], myIndex: number, t: CricketTarget): number {
 ...
 */
function denialUrgency(myMarks: Record<CricketTarget, number>, players: CricketPlayer[], myIndex: number, t: CricketTarget): number {
...
export function cricketPickTarget(
  ...
  const weights =
    ...
    scoreDiff >=  LOCKDOWN_THRESHOLD ? LOCKDOWN_WEIGHTS :
    RACE_WEIGHTS;
  ...
  for (const t of TARGETS) {
    ...
    if (iClosed(myMarks, t) && allOpponentsClosed(allPlayers, myIndex, t)) continue;
    ...
    const cv = (iClosed(myMarks, t) || allOpponentsClosed(allPlayers, myIndex, t))
      ? 0
