...
export function HistoryRow({
  ...
  return (
    ...
    >
      <span className={`text-[clamp(0.7rem,1.2vw,1.1rem)] font-bold tabular-nums text-center leading-none ${
      ...
      {[0, 1, 2].map((j) => {
        ...
        return (
          <div key={j} className="history-dart-cell" data-state={cellState}>
            {d ? (() => {
              const label = d.shortName === "BULL" ? "B" : d.shortName === "DBULL" ? "DB" : d.shortName;
              const isSingle = !label.startsWith("T") && !label.startsWith("D") && label !== "B";
              return isSingle ? (
                <span className={`text-[clamp(0.85rem,1.5vw,1.6rem)] font-black leading-none tabular-nums ${
                ...
                <span className={`text-[clamp(0.85rem,1.5vw,1.6rem)] font-black leading-none tabular-nums ${
                  d.state === "bust" ? "text-state-bust" :
                  d.state === "scored" ? "text-[var(--color-game-accent)]" : "text-zinc-600"
                }`}>{label}</span>
              );
            })() : isNextSlot ? (
