...
export function segmentCenter(segmentId: SegmentID): { x: number; y: number } {
  ...
  const sectorIndex = CLOCKWISE_NUMBERS.indexOf(number as (typeof CLOCKWISE_NUMBERS)[number]);
  const angleDeg = sectorIndex * 18;
  const rad = (angleDeg * Math.PI) / 180;

  const r =
    zoneCode === 1 ? ZONE_MID.treble
    : zoneCode === 3 ? ZONE_MID.double
    : zoneCode === 2 ? ZONE_MID.outerSingle
