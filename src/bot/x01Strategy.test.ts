...
describe("Rule 2 — score 50: aim DBL_BULL to finish", () => {
  ...
  it("aims DBL_BULL at score 50 in doubleOut", () => {
    expect(x01PickTarget(50, { ...base, doubleOut: true }, true)).toBe(SegmentID.DBL_BULL);
  });

  it("aims DBL_BULL at score 50 in masterOut", () => {
    expect(x01PickTarget(50, { ...base, masterOut: true }, true)).toBe(SegmentID.DBL_BULL);
  });
});
...
describe("Rule 3 — masterOut: triple finish when score ≤ 60 and divisible by 3", () => {
  ...
  it("does NOT apply when score is > 60", () => {
    ...
    expect(Object.values(SegmentID) as number[]).toContain(x01PickTarget(63, opts, true));
  });
});
...
describe("Rule 4 — doubleOut/masterOut: double finish when score ≤ 40 and even", () => {
  it("aims DBL_20 at score 40 with doubleOut", () => {
    expect(x01PickTarget(40, { ...base, doubleOut: true }, true)).toBe(SegmentID.DBL_20);
  });

  it("aims DBL_16 at score 32 with doubleOut", () => {
    expect(x01PickTarget(32, { ...base, doubleOut: true }, true)).toBe(SegmentID.DBL_16);
  });

  it("aims DBL_1 at score 2 with doubleOut", () => {
    expect(x01PickTarget(2, { ...base, doubleOut: true }, true)).toBe(SegmentID.DBL_1);
  });

  it("aims DBL_20 at score 40 with masterOut", () => {
    expect(x01PickTarget(40, { ...base, masterOut: true }, true)).toBe(SegmentID.DBL_20);
  });

  it("aims DBL_16 at score 32 with masterOut", () => {
    expect(x01PickTarget(32, { ...base, masterOut: true }, true)).toBe(SegmentID.DBL_16);
  });
  ...
  it("does NOT apply at score 40 in standard out (falls to Rule 6)", () => {
    ...
    expect(x01PickTarget(42, { ...base, doubleOut: true }, true)).not.toBe(SegmentID.DBL_20);
  });
});
...
describe("Rule 5 — doubleOut/masterOut: aim OUTER_1 to leave even finish", () => {
  it("aims OUTER_1 at score 39 with doubleOut", () => {
    expect(x01PickTarget(39, { ...base, doubleOut: true }, true)).toBe(SegmentID.OUTER_1);
  });

  it("aims OUTER_1 at score 1 with doubleOut", () => {
    expect(x01PickTarget(1, { ...base, doubleOut: true }, true)).toBe(SegmentID.OUTER_1);
  });

  it("aims OUTER_1 at score 37 with masterOut", () => {
    expect(x01PickTarget(37, { ...base, masterOut: true }, true)).toBe(SegmentID.OUTER_1);
  });
});
...
describe("Rule 7 — default target for high scores", () => {
  it("aims DBL_BULL when splitBull is off (combined bull = bigger target)", () => {
    expect(x01PickTarget(501, { ...base, splitBull: false }, true)).toBe(SegmentID.DBL_BULL);
    expect(x01PickTarget(200, { ...base, splitBull: false }, true)).toBe(SegmentID.DBL_BULL);
  });

  it("aims TRP_20 when splitBull is on (outer bull only = 25, T20 = 60 is better)", () => {
    expect(x01PickTarget(501, { ...base, splitBull: true }, true)).toBe(SegmentID.TRP_20);
    expect(x01PickTarget(200, { ...base, splitBull: true }, true)).toBe(SegmentID.TRP_20);
  });

  it("aims DBL_BULL by default for doubleOut games above finish range", () => {
    expect(x01PickTarget(200, { ...base, doubleOut: true }, true)).toBe(SegmentID.DBL_BULL);
  });

  it("aims TRP_20 for doubleOut games with splitBull on", () => {
    expect(x01PickTarget(200, { ...base, doubleOut: true, splitBull: true }, true)).toBe(SegmentID.TRP_20);
  });
});
...
describe("Rule 2 — single-out outchart (split bull off)", () => {
  ...
  it("180 → TRP_20", () => expect(x01PickTarget(180, std, true)).toBe(SegmentID.TRP_20));
  it("170 → TRP_20 (T20 T20 B)", () => expect(x01PickTarget(170, std, true)).toBe(SegmentID.TRP_20));
  it("171 → TRP_19 (T19 T19 T19 is first path)", () => expect(x01PickTarget(171, std, true)).toBe(SegmentID.TRP_19));
  it("160 → DBL_BULL (B B T20)", () => expect(x01PickTarget(160, std, true)).toBe(SegmentID.DBL_BULL));
  it("145 → TRP_15 (T15 B B)", () => expect(x01PickTarget(145, std, true)).toBe(SegmentID.TRP_15));
  it("150 → DBL_BULL (B B B)", () => expect(x01PickTarget(150, std, true)).toBe(SegmentID.DBL_BULL));
  it("110 → TRP_20 (T20 B)", () => expect(x01PickTarget(110, std, true)).toBe(SegmentID.TRP_20));
  it("107 → TRP_19 (T19 B)", () => expect(x01PickTarget(107, std, true)).toBe(SegmentID.TRP_19));
  it("100 → DBL_BULL (B B)", () => expect(x01PickTarget(100, std, true)).toBe(SegmentID.DBL_BULL));
  it("97  → TRP_19 (T19 D20)", () => expect(x01PickTarget(97, std, true)).toBe(SegmentID.TRP_19));
  it("91  → TRP_17 (T17 D20)", () => expect(x01PickTarget(91, std, true)).toBe(SegmentID.TRP_17));
  it("85  → TRP_15 (T15 D20)", () => expect(x01PickTarget(85, std, true)).toBe(SegmentID.TRP_15));
  it("70  → OUTER_20 (20 B)", () => expect(x01PickTarget(70, std, true)).toBe(SegmentID.OUTER_20));
  it("69  → OUTER_19 (19 B)", () => expect(x01PickTarget(69, std, true)).toBe(SegmentID.OUTER_19));
  it("60  → TRP_20 (1-dart finish)", () => expect(x01PickTarget(60, std, true)).toBe(SegmentID.TRP_20));
  it("57  → TRP_19 (1-dart finish)", () => expect(x01PickTarget(57, std, true)).toBe(SegmentID.TRP_19));
  it("50  → DBL_BULL (1-dart finish)", () => expect(x01PickTarget(50, std, true)).toBe(SegmentID.DBL_BULL));
  it("48  → TRP_16 (1-dart finish)", () => expect(x01PickTarget(48, std, true)).toBe(SegmentID.TRP_16));
  it("42  → TRP_14 (1-dart finish)", () => expect(x01PickTarget(42, std, true)).toBe(SegmentID.TRP_14));
  it("41  → OUTER_9 (9 D16)", () => expect(x01PickTarget(41, std, true)).toBe(SegmentID.OUTER_9));
...
describe("Rule interactions and edge cases", () => {
  it("doubleIn + unopened takes priority over everything else (score 50)", () => {
    ...
    expect(x01PickTarget(50, { ...base, doubleIn: true }, false)).toBe(SegmentID.DBL_20);
  });
  ...
  it("masterOut at score 60 → TRP_20 (Rule 3), not DBL_20 (Rule 4 requires ≤40)", () => {
    expect(x01PickTarget(60, { ...base, masterOut: true }, true)).toBe(SegmentID.TRP_20);
  });
