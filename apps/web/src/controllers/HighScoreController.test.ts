...
describe("HighScoreController", () => {
  ...
  it("onDartHit calls addDart and emits dart_hit", () => {
    const addDart = vi.fn();
    vi.mocked(useHighScoreStore.getState).mockReturnValue({ addDart } as never);
    ctrl.onDartHit(s20);
  ...
  it("onNextTurn calls nextTurn and emits next_turn", () => {
    ...
    vi.mocked(useHighScoreStore.getState).mockReturnValue({
      nextTurn,
    } as never);
    ctrl.onNextTurn();
  ...
  it("dart_hit event has segment property", () => {
    ...
    const call = vi
      ...
      .mock.calls.find((c) => c[0] === "dart_hit");
  ...
  it("next_turn event has empty payload", () => {
    ...
    const call = vi
      .mocked(gameEventBus.emit)
      .mock.calls.find((c) => c[0] === "next_turn");
    expect(call).toBeDefined();
